...
const model = {
    ...
    getLog(log_id, cb){
        conn.query("SELECT firstName, lastName, day, month, year, bcalories, lcalories, dcalories, scalories, breakfast, lunch, dinner, snacks FROM FT_Users JOIN FT_FoodLogs on FT_Users.id = FT_FoodLogs.id WHERE log_id = ? ORDER BY log_id DESC", log_id, (err, data) => {
            cb(err, data); 
        })
